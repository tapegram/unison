...
newtype PinBoard a
  = PinBoard (MVar (HashMap Int (Bucket a)))

new :: MonadIO m => m (PinBoard a)
new =
  liftIO (PinBoard <$> newMVar HashMap.empty)
...
pinWith (PinBoard boardVar) n x = liftIO do
  modifyMVar boardVar \board ->
    swap <$> getCompose (HashMap.alterF alter n board)
  where
    ...
    finalizer :: IO ()
    finalizer =
      modifyMVar_ boardVar (HashMap.alterF (maybe (pure Nothing) bucketCompact) n)

debugDump :: MonadIO m => (a -> Text) -> PinBoard a -> m ()
debugDump f (PinBoard boardVar) = liftIO do
  board <- readMVar boardVar
  contents <- (traverse . traverse) bucketToList (HashMap.toList board)
